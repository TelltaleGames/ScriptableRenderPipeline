...
/*
        float height0 = SAMPLE_UVMAPPING_TEXTURE2D(_HeightMap0, SAMPLER_HEIGHTMAP_IDX, layerTexCoord.base0).r - _HeightOffset0 + (blendMasks.r*2.0 - 1.0);
        float height1 = SAMPLE_UVMAPPING_TEXTURE2D(_HeightMap1, SAMPLER_HEIGHTMAP_IDX, layerTexCoord.base1).r - _HeightOffset1 + (blendMasks.g*2.0 - 1.0);
        float height2 = SAMPLE_UVMAPPING_TEXTURE2D(_HeightMap2, SAMPLER_HEIGHTMAP_IDX, layerTexCoord.base2).r - _HeightOffset2 + (blendMasks.b*2.0 - 1.0);
        float height3 = SAMPLE_UVMAPPING_TEXTURE2D(_HeightMap3, SAMPLER_HEIGHTMAP_IDX, layerTexCoord.base3).r - _HeightOffset3 + (blendMasks.a*2.0 - 1.0);
*/
        float height0 = SAMPLE_UVMAPPING_TEXTURE2D(_BaseColorMap0, sampler_BaseColorMap0, layerTexCoord.base0).a - _HeightOffset0 + (blendMasks.r*2.0 - 1.0);
        float height1 = SAMPLE_UVMAPPING_TEXTURE2D(_BaseColorMap1, sampler_BaseColorMap0, layerTexCoord.base1).a - _HeightOffset1 + (blendMasks.g*2.0 - 1.0);
        float height2 = SAMPLE_UVMAPPING_TEXTURE2D(_BaseColorMap2, sampler_BaseColorMap0, layerTexCoord.base2).a - _HeightOffset2 + (blendMasks.b*2.0 - 1.0);
        float height3 = SAMPLE_UVMAPPING_TEXTURE2D(_BaseColorMap3, sampler_BaseColorMap0, layerTexCoord.base3).a - _HeightOffset3 + (blendMasks.a*2.0 - 1.0);
...
void GetSurfaceAndBuiltinData(FragInputs input, float3 V, inout PositionInputs posInput, out SurfaceData surfaceData, out BuiltinData builtinData)
{
    ...
    //surfaceData.baseColor = blendMasks.rgb;
    //surfaceData.baseColor = float4(weights);
