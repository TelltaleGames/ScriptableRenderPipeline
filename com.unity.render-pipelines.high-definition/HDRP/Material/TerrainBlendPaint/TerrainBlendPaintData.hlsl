// Extends (not based on) LitData.hlsl.
...
float4 GetBlendMask(LayerTexCoord layerTexCoord, float4 vertexColor, bool useLodSampling = false, float lod = 0)
{
    ...
    float4 blendMasks = tex2D(_Control, layerTexCoord.blendMask.uv);
...
void ComputeLayerWeights(FragInputs input, LayerTexCoord layerTexCoord, float4 inputAlphaMask, float4 blendMasks, out float outWeights[_MAX_LAYER])
{
    ...
    //#if LAYERS_HEIGHTMAP_ENABLE
        ...
        blendMasks.a = max(0.0, 1.0 - blendMasks.r - blendMasks.g - blendMasks.b);
        //float totalweight = dot(blendMasks, half4(1,1,1,1));
        //blendMasks /= max(totalweight, 1e-3f);

        float blending = max(_HeightTransition, 0.01);

        float height0 = tex2D(_Splat0, TRANSFORM_TEX(float2(layerTexCoord.blendMask.uv.y,layerTexCoord.blendMask.uv.x), _Splat0)).a - _HeightOffset0 + (blendMasks.r*2.0 - 1.0);
        float height1 = tex2D(_Splat1, TRANSFORM_TEX(float2(layerTexCoord.blendMask.uv.y,layerTexCoord.blendMask.uv.x), _Splat1)).a - _HeightOffset1 + (blendMasks.g*2.0 - 1.0);
        float height2 = tex2D(_Splat2, TRANSFORM_TEX(float2(layerTexCoord.blendMask.uv.y,layerTexCoord.blendMask.uv.x), _Splat2)).a - _HeightOffset2 + (blendMasks.b*2.0 - 1.0);
        float height3 = tex2D(_Splat3, TRANSFORM_TEX(float2(layerTexCoord.blendMask.uv.y,layerTexCoord.blendMask.uv.x), _Splat3)).a - _HeightOffset3 + (blendMasks.a*2.0 - 1.0);
        ...
        float w3 = max(height3 - layermax, 1e-3f);
        float wsum = w0 + w1 + w2 + w3;

        //ComputeMaskWeights(blendMasks, outWeights);

        outWeights[0] = w0/wsum;
        outWeights[1] = w1/wsum;
        #if _LAYER_COUNT > 2
            outWeights[2] = w2/wsum;
        #else
            outWeights[2] = 0.0;
        #endif
        #if _LAYER_COUNT > 3
            outWeights[3] = w3/wsum;
        #else
            outWeights[3] = 0.0;
        #endif
    //#endif
...
float3 ComputeMainBaseColorInfluence(float influenceMask, float3 baseColor0, float3 baseColor1, float3 baseColor2, float3 baseColor3, LayerTexCoord layerTexCoord, float inputMainLayerMask, float weights[_MAX_LAYER])
{
    ...
    float3 baseMeanColor0 = tex2D(_Splat0, TRANSFORM_TEX(float2(layerTexCoord.base0.uv.y,layerTexCoord.base0.uv.x), _Splat0)).rgb *_BaseColor0.rgb;
    float3 baseMeanColor1 = tex2D(_Splat1, TRANSFORM_TEX(float2(layerTexCoord.base0.uv.y,layerTexCoord.base0.uv.x), _Splat1)).rgb *_BaseColor1.rgb;
    float3 baseMeanColor2 = tex2D(_Splat2, TRANSFORM_TEX(float2(layerTexCoord.base0.uv.y,layerTexCoord.base0.uv.x), _Splat2)).rgb *_BaseColor2.rgb;
    float3 baseMeanColor3 = tex2D(_Splat3, TRANSFORM_TEX(float2(layerTexCoord.base0.uv.y,layerTexCoord.base0.uv.x), _Splat3)).rgb *_BaseColor3.rgb;
...
void GetSurfaceAndBuiltinData(FragInputs input, float3 V, inout PositionInputs posInput, out SurfaceData surfaceData, out BuiltinData builtinData)
{
    ...
    // nned to look up the textures from the Terrain UI ourselves, since we can't edit the GetSurfaceData functions above.
    surfaceData0.baseColor = tex2D(_Splat0, TRANSFORM_TEX(float2(layerTexCoord.base0.uv.y,layerTexCoord.base0.uv.x), _Splat0)).rgb *_BaseColor0.rgb;
    surfaceData1.baseColor = tex2D(_Splat1, TRANSFORM_TEX(float2(layerTexCoord.base0.uv.y,layerTexCoord.base0.uv.x), _Splat1)).rgb *_BaseColor1.rgb;
    surfaceData2.baseColor = tex2D(_Splat2, TRANSFORM_TEX(float2(layerTexCoord.base0.uv.y,layerTexCoord.base0.uv.x), _Splat2)).rgb *_BaseColor2.rgb;
    surfaceData3.baseColor = tex2D(_Splat3, TRANSFORM_TEX(float2(layerTexCoord.base0.uv.y,layerTexCoord.base0.uv.x), _Splat3)).rgb *_BaseColor3.rgb;
    normalTS0 = 2.0f * tex2D(_Normal0, TRANSFORM_TEX(float2(layerTexCoord.base0.uv.y,layerTexCoord.base0.uv.x), _Splat0)).rgb - 1.0f;
    normalTS1 = 2.0f * tex2D(_Normal1, TRANSFORM_TEX(float2(layerTexCoord.base0.uv.y,layerTexCoord.base0.uv.x), _Splat1)).rgb - 1.0f;
    normalTS2 = 2.0f * tex2D(_Normal2, TRANSFORM_TEX(float2(layerTexCoord.base0.uv.y,layerTexCoord.base0.uv.x), _Splat2)).rgb - 1.0f;
    normalTS3 = 2.0f * tex2D(_Normal3, TRANSFORM_TEX(float2(layerTexCoord.base0.uv.y,layerTexCoord.base0.uv.x), _Splat3)).rgb - 1.0f;
...
#endif
    {
        //surfaceData.baseColor = BlendLayeredVector3(surfaceData0.baseColor, surfaceData1.baseColor, surfaceData2.baseColor, surfaceData3.baseColor, weights);
    ...
    surfaceData.perceptualSmoothness = 0.0;//SURFACEDATA_BLEND_SCALAR(surfaceData, perceptualSmoothness, weights);
    ...
    //surfaceData.baseColor = float4(_BaseColor1.rgb,1.0);
    //surfaceData.baseColor = surfaceData.tangentWS;
    //surfaceData.baseColor = float4(weights);//blendMasks.rgb;
    //surfaceData.baseColor = surfaceData1.baseColor;
    //surfaceData.baseColor = SAMPLE_UVMAPPING_TEXTURE2D(_Splat3, sampler_Splat3, layerTexCoord.base3).rgb;
    //surfaceData.baseColor = float4( fmod(layerTexCoord.base0.uv, 1.0), 0, 1 );
}
