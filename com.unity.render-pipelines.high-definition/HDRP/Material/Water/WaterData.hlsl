...
#define SAMPLER_NORMALMAP1_IDX sampler_NormalMap1
#define SAMPLER_NORMALMAP2_IDX sampler_NormalMap2
#endif
...
// include WaterLayer to define GetSurfaceData
...
void GetSurfaceAndBuiltinData(FragInputs input, float3 V, inout PositionInputs posInput, out SurfaceData surfaceData, out BuiltinData builtinData)
{
    ...
    float3 bentNormalWS;
    float presence;
    float alpha = GetSurfaceData(input, layerTexCoord, surfaceData, presence, normalTS, bentNormalTS);

    GetNormalWS(input, V, normalTS, surfaceData.normalWS);

    // Dim baseColor based on fresnel, since baseColor for water represents color of murk, beneath the surface.
    float fresnel = pow(1.0-dot(V,surfaceData.normalWS),4.0);
    surfaceData.baseColor = lerp(surfaceData.baseColor, float3(0,0,0), fresnel);
...
#if defined(_ENABLESPECULAROCCLUSION)
